...
# Tarea 2 de la Sesion 4
# Filtrado de datos 

# Filtrado de productos según criterios específicos
def filtrar_productos(productos, min_precio, max_precio, categoria):
    ...
    for producto in productos:
        # 1. Verificación de todos los criterios
        precio_valido = min_precio <= producto['precio'] <= max_precio
        tiene_stock = producto['stock'] > 0
        categoria_correcta = producto['categoria'] == categoria
        
        # Si cumple todos los criterios, se agrega a la lista
        if precio_valido and tiene_stock and categoria_correcta:
            productos_filtrados.append(producto)
    
    return productos_filtrados

def resultados_filtros():
    # Casos de prueba
    productos = [
        {"nombre": "Tornillos", "precio": 1200, "stock": 5, "categoria": "Ferreteria"},
        {"nombre": "Computadoras", "precio": 25, "stock": 10, "categoria": "Electrónica"},
        {"nombre": "Cuadernos", "precio": 30, "stock": 0, "categoria": "Libreria"},
        {"nombre": "Vasos", "precio": 300, "stock": 3, "categoria": "Cafeteri"}
    ]
...
if __name__ == "__resultados_filtros__":
    resultados_filtros()